...
def schedule_strategy():
    ...
    schedule.every().hour.at(":03").do(run_strategy, timeframe='1h')  # 1h candle closes at hh:00
    schedule.every(4).hours.at(":05").do(run_strategy, timeframe='4h')  # 4h candle closes at hh:00, hh:04, etc.
    schedule.every().day.at("00:07").do(run_strategy, timeframe='1d')  # 1d candle closes at 00:00

    # Schedule confirmation for pending positions
    schedule.every().hour.at(":02").do(confirm_and_apply)  # Confirmation runs shortly after candle closes

    # Schedule check_exit_conditions to run every hour
    schedule.every().hour.at(":01").do(check_exit_conditions)
